...
pub async fn run_daemon(session: &str) {
    ...
    let _ = fs::remove_file(socket_dir.join(format!("{}.stream", session)));
    ...
    let result = run_socket_server(&socket_path, session, idle_timeout_ms).await;
                ...
                match accept_result {
                    Ok((stream, _)) => {
                        ...
                        tokio::spawn(async move {
                            handle_connection(stream, state, reset_tx, cn).await;
                        });
                    }
                    Err(e) => {
