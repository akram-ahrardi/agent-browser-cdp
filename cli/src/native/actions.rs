...
async fn handle_state(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
    ...
    let js = format!(
        r#"(()=>{{document.querySelectorAll('[data-ab-ref]').forEach(e=>e.removeAttribute('data-ab-ref'));var sel='a[href],button,input:not([type=hidden]),textarea,select,[role=button],[role=link],[role=tab],[onclick],[tabindex]:not([tabindex="-1"]),[contenteditable]';var els=document.querySelectorAll(sel);var out=[];for(var i=0;i<els.length&&out.length<{max};i++){{var e=els[i];if(!e.offsetParent)continue;var idx=out.length+1;e.setAttribute('data-ab-ref',idx);var tag=e.tagName.toLowerCase();var ce=e.getAttribute('contenteditable');if(ce!==null&&(ce===''||ce==='true')){{tag+='[contenteditable]';}}var t=e.getAttribute('type')||'';var txt=(e.innerText||e.value||e.placeholder||e.getAttribute('aria-label')||'').substring(0,80).replace(/\s+/g,' ').trim();var href=e.href?e.href.substring(0,100):'';out.push([idx,tag,t,txt,href]);}}return JSON.stringify(out);}})()"#,
        max = max
    );
